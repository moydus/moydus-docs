    ...
    <>
      ...
      {isOpen && (
        <button
          type="button"
          onClick={() => setIsOpen(false)}
          className="fixed bg-white dark:bg-gray-800 rounded-full top-4 right-4 w-8 h-8 flex items-center justify-center text-gray-500 dark:text-gray-400 hover:text-gray-600 dark:hover:text-gray-200 shadow-md z-80 lg:hidden"
      ...
      <div
        className={cn(
          'fixed top-0 left-0 bottom-0 w-[20rem] bg-white dark:bg-[#0e0e0f] border-r border-gray-100 dark:border-white/10 z-70 transition-transform duration-300 ease-in-out lg:hidden',
          isOpen ? 'translate-x-0' : '-translate-x-full',
        )}
      >
        <div className="flex flex-col h-full">
          <div className="flex items-center justify-between px-4 pt-6 pb-4">
            <a href="/" className="flex items-center gap-2 select-none" onClick={() => setIsOpen(false)}>
              <img
                src="/logo/light.svg"
                alt="Moydus"
